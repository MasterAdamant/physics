...
namespace Ung
{
	int UNG_MAGIC_5 = 'a' | ('r' << 8) | ('0' << 16) | ('5' << 24);
	int UNG_MAGIC_6 = 'a' | ('r' << 8) | ('0' << 16) | ('6' << 24);
	...
	void getUngInfo(string file_path, int &magic)
	{
		File file = new File();
	...
	{
		int magic;
		getUngInfo(file, magic);

		if(magic == 0)
			return;

		if(magic != UNG_MAGIC_5 && magic != UNG_MAGIC_6)
			return;
		...
		string old_archiver_path = bin_dir + "ung_2.19.1.2_x64" + bin_ext;
		string new_archiver_path = bin_dir + "ung_2.20.0.0_x64" + bin_ext;
		...
		if((unpack_result == 0) && (ung_files_num > 0))
		{
			...
			string cmd_pack = format("%s -o %s %s", new_archiver_path, file, ung_files_list);

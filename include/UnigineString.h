...
public:
	...
	bool isDynamicallyAllocated() const { return dynamic != 0; }
	...
	UNIGINE_INLINE int findChar(const char *s) const
	{
		if (s == nullptr)
			return -1;
		int size = (int)strlen(s);
	...
	UNIGINE_INLINE int findChar(const String &s) const
	{
		if (s.empty())
			return -1;
		for (int i = 0; i < length; i++)
	...
	UNIGINE_INLINE bool containsChar(const char *s) const { return findChar(s) >= 0; }
	UNIGINE_INLINE bool containsChar(const String &s) const { return findChar(s) >= 0; }
	...
	{
		return String::equal(str, "1") || (String::compare(str, "true", false) == 0);
	}
	static int atoi(const char *str, bool &error)
	{
		error = false;

		if (String::compare(str, "true", false) == 0)
			return 1;
		if (String::compare(str, "false", false) == 0)
			return 0;
	...
	UNIGINE_INLINE static int utf8ToUnicode(const char *src, Vector<wchar_t> &dest)
	{
		const char *s = src;
		unsigned int code = 0;
		while (*s)
		{
			...
			if (code < 0x10000)
			{
				dest.append((wchar_t)code);
			} else
			{
				code -= 0x10000;
				dest.append((wchar_t)((code >> 10) + 0xd800));
				dest.append((wchar_t)((code & 0x3ff) + 0xdc00));
			}
		}
		dest.append('\0');
		return dest.size();
...
public:
	...
	friend UNIGINE_INLINE void swap(StringPtr &v0, StringPtr &v1)
	{
		v0.swap(v1);
	}
...
class UNIGINE_API StringPoolPtr
{
public:
	...
	UNIGINE_INLINE void swap(StringPoolPtr &s)
	{
		std::swap(ptr, s.ptr);
	}
	friend UNIGINE_INLINE void swap(StringPoolPtr &v0, StringPoolPtr &v1)
	{
		v0.swap(v1);
	}

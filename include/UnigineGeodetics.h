...
{
public:
	typedef const GeodeticPivot ConstGeodeticPivot;
	...
	enum UP_AXIS
	{
		UP_AXIS_GEOCENTRIC_NORMAL = 0,
		UP_AXIS_GEODETIC_NORMAL,
	};
	...
	Math::dmat4 toWorld(const Math::dvec3 &geodetic_coords, GeodeticPivot::UP_AXIS up_axis = GeodeticPivot::UP_AXIS_GEOCENTRIC_NORMAL);
	Math::dmat4 toWorldPreserveRotation(const Math::dmat4 &world_transform, const Math::dvec3 &geodetic_coords, GeodeticPivot::UP_AXIS up_axis = GeodeticPivot::UP_AXIS_GEOCENTRIC_NORMAL);
	Math::mat4 toWorldPreserveRotation(const Math::mat4 &world_transform, const Math::dvec3 &geodetic_coords, GeodeticPivot::UP_AXIS up_axis = GeodeticPivot::UP_AXIS_GEOCENTRIC_NORMAL);
...
typedef Ptr<const GeodeticPivot> ConstGeodeticPivotPtr;
typedef const GeodeticPivot ConstGeodeticPivot;

...
struct alignas(16) vec3
{
	...
	UNIGINE_INLINE float minXY() const { return Math::min(x, y); }
	/// <summary>Compares all components of the vector and returns the smallest value.</summary>
	UNIGINE_INLINE float min() const { return Math::min(minXY(), z); }
...
UNIGINE_INLINE bool compare(const vec3 &v0, const vec3 &v1)
{
...
UNIGINE_INLINE bool compare(const vec3 &v0, const vec3 &v1, float epsilon)
{
...
UNIGINE_INLINE bool operator==(const vec3 &v0, const vec3 &v1) { return compare(v0, v1); }
/// <summary>Vector not equal comparison.</summary>
UNIGINE_INLINE bool operator!=(const vec3 &v0, const vec3 &v1) { return !compare(v0, v1); }

/// <summary>Vector less than comparison.</summary>
UNIGINE_INLINE bool operator<(const vec3 &v0, const vec3 &v1) { return v0.x < v1.x && v0.y < v1.y && v0.z < v1.z; }
...
{
	#ifdef USE_SSE
		ret.vec = _mm_mad_ps(v0.vec, _mm_set_ps1(v1), v2.vec);
	#else
		ret.x = v0.x * v1 + v2.x;
		ret.y = v0.y * v1 + v2.y;
		ret.z = v0.z * v1 + v2.z;
...
{
	#ifdef USE_SSE
		ret.vec = _mm_mad_ps(v0.vec, v1.vec, v2.vec);
	#else

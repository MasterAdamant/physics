...
struct FlatHashMapPolicy
{
	...
	{
		using std::swap;
		swap(const_cast<HashType &>(s0.hash), const_cast<HashType &>(s1.hash));
		swap(const_cast<Key &>(s0.key), const_cast<Key &>(s1.key));
		swap(s0.data, s1.data);
	}
...
struct BucketHashMapPolicy
{
	...
	UNIGINE_INLINE static void replace(Slot &slot, const Slot &from) { slot->data = from->data; }
	UNIGINE_INLINE static void replace(Slot &slot, Slot &&from) { using std::swap; swap(slot, from); }

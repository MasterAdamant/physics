...
public:
	...
	virtual Bank * loadBankFromMemory(const char *buffer, int size) = 0;
...
public:
	...
	UNIGINE_INLINE static FMOD *get()
	{
		static FMOD *instance = nullptr;
		static EventConnection connection;
		static Mutex m;
		...
		if (instance == nullptr)
		{
			...
			if (instance == nullptr)
			{
				...
				{
					Engine::get()->getEventPluginRemoved().connect(
						connection,
						[](const char *name)
						{
							if (String::equal(name, "UnigineFMOD"))
							{
								instance = nullptr;
				...
				instance = Unigine::Engine::get()->getPlugin<FMOD>("UnigineFMOD");
			}
		}
		return instance;
	}

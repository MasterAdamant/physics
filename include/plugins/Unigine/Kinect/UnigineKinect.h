...
public:
	...
	UNIGINE_INLINE static Kinect *get()
	{
		static Kinect *instance = nullptr;
			...
			if (instance == nullptr)
			{
				if (!connection.isValid())
				{
					Engine::get()->getEventPluginRemoved().connect(
						...
						[](const char *name)
						{
							if (String::equal(name, "UnigineKinect"))
							{
								instance = nullptr;
				...
				instance = Unigine::Engine::get()->getPlugin<Kinect>("UnigineKinect");
			}
		}
		return instance;
	}

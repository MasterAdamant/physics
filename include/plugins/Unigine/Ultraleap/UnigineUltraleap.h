...
public:
	...
	UNIGINE_INLINE static Ultraleap *get()
	{
		static Ultraleap *instance = nullptr;
			...
			if (instance == nullptr)
			{
				if (!connection.isValid())
				{
					Engine::get()->getEventPluginRemoved().connect(
						...
						[](const char *name)
						{
							if (String::equal(name, "UnigineUltraleap"))
							{
								instance = nullptr;
				...
				instance = Unigine::Engine::get()->getPlugin<Ultraleap>("UnigineUltraleap");
			}
		}
		return instance;
	}

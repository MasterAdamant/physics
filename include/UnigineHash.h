...
public:
	...
	{
		const auto c = static_cast<size_t>(capacity);
		return static_cast<Counter>(pos >= c ? pos % c : pos);
	}
...
public:
	...
	friend UNIGINE_INLINE void swap(Hash &v0, Hash &v1)
	{
		v0.swap(v1);
	}
	...
	void clear()
	{
		...
		traverse_full_slots([](Slot *slot) { Policy::destruct(*slot); });
		length = 0;
		reset_controls();
	}
	...
	void reserve(size_t size)
	{
		...
		if (new_capacity <= static_cast<size_t>(capacity))
			return;
...
protected:
	...
	{
		assert(pos >= 0 && static_cast<size_t>(pos) < (capacity - 1));
		ctrl[pos] = v;
		// Special for correct ring traverse
		if (static_cast<size_t>(pos) < get_tail_elements())
			ctrl[capacity + pos] = v;
	}
	...
	Counter rehash_expand_with_remap(Counter remap, Counter new_capacity)
	{
		...
		if (static_cast<size_t>((char *)new_slots - (char *)old_slots) < sizeof(Slot))
		{
...
protected:
	...
	UNIGINE_INLINE void destroy_slot(Slot *slot, Control *ctrl)
	{
		...
		{
			constexpr Counter WIDTH = Group::Width;
			Counter start;
			Counter deleted;
			Counter num;
			...
			if ((deleted > 0 && is_rehash_needed(deleted + length)) ||
				...
				(deleted > (WIDTH * 2) && num >= (capacity / 2)) ||
				(deleted > (WIDTH * 2) && num > (WIDTH * 8)))
			{

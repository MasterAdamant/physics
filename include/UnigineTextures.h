...
{
public:
	typedef const Texture ConstTexture;
	...
	bool create(const Ptr<Image> &image, int flags = Texture::SAMPLER_FILTER_LINEAR, int skip_mipmaps = 0, int format = -1);
...
typedef Ptr<const Texture> ConstTexturePtr;
typedef const Texture ConstTexture;
...
{
public:
	typedef const StructuredBuffer ConstStructuredBuffer;
	static constexpr bool HAS_OWNERSHIP = true;
...
typedef Ptr<const StructuredBuffer> ConstStructuredBufferPtr;
typedef const StructuredBuffer ConstStructuredBuffer;
...
{
public:
	typedef const RenderTarget ConstRenderTarget;
	static constexpr bool HAS_OWNERSHIP = true;
...
typedef Ptr<const RenderTarget> ConstRenderTargetPtr;
typedef const RenderTarget ConstRenderTarget;
...
{
public:
	typedef const TextureRamp ConstTextureRamp;
	static constexpr bool HAS_OWNERSHIP = true;
...
typedef Ptr<const TextureRamp> ConstTextureRampPtr;
typedef const TextureRamp ConstTextureRamp;

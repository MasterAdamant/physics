...
public:
	...
	virtual int findFile(const char *path) const = 0;
...
{
public:
	typedef const FileSystemMount ConstFileSystemMount;
	...
	void setExclusiveFilters(const Vector<String> &filters);
	void getExclusiveFilters(Vector<String> &filters) const;
	Vector<String> getExclusiveFilters() const;
	void setIgnoreFilters(const Vector<String> &filters);
	...
	void getGuidsDBIgnoreFilters(Vector<String> &filters) const;
...
typedef Ptr<const FileSystemMount> ConstFileSystemMountPtr;
typedef const FileSystemMount ConstFileSystemMount;
...
public:
	...
	static Ptr<FileSystemMount> createMount(const char *absolute_path, const char *virtual_path, int access, const Vector<String> &exclusive_filters = {}, const Vector<String> &ignore_filters = {}, const Vector<String> &guidsdb_ignore_filters = {}, bool save_file = true);
	...
	static bool isExternalFile(const char *path);
	static bool isExternalFile(const UGUID &guid);

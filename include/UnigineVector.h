...
public:
	...
	public:
		...
		template<typename T>
		UNIGINE_INLINE bool operator==(const ReverseIteratorTemplate<T> &it) const { return ptr == it.ptr; }
		template<typename T>
		UNIGINE_INLINE bool operator!=(const ReverseIteratorTemplate<T> &it) const { return ptr != it.ptr; }
		template<typename T>
...
public:
	...
	friend UNIGINE_INLINE void swap(Vector &v0, Vector &v1)
	{
		v0.swap(v1);
	}
